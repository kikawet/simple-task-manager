...
<template>
  <div
    class="bg-background w-fit px-4 border border-textColor-300 h-16 shadow-lg rounded-md gap-4 flex flex-row items-center justify-center"
  >
    <section class="flex flex-col items-start justify-end gap-1">
      <h1 class="text-base font-semibold text-zinc-800 antialiased">{{ msg }}</h1>
    </section>
    <section @click="$emit('closed')" class="w-5 flex flex-col items-center justify-start">
      <svg
        width="100%"
        viewBox="0 0 15 15"
        fill="none"
